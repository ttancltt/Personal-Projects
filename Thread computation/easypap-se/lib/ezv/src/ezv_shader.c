...
static char *file_load (const char *file)
{
  ...
  char filename[1024];
  snprintf (filename, sizeof (filename), "%s/share/ezv/shaders/%s", ezv_prefix,
            file);
                                 ...
                                 GLuint *location)
{
  ...
  if (loc == GL_INVALID_INDEX)
    exit_with_error ("glGetUniformLocation for %s\n", name);
  *location = loc;
}
...
#ifdef __linux__
void ezv_shader_bind_ssbo (GLuint program, const char *name,
                           GLuint blockbinding)
{
  GLuint index =
      glGetProgramResourceIndex (program, GL_SHADER_STORAGE_BLOCK, name);
  if (index == GL_INVALID_INDEX)
    exit_with_error ("glGetProgramResourceIndex for %s", name);

  // Link Binding indices to binding points
  glShaderStorageBlockBinding (program, index, blockbinding);
}
#endif