...
void ezv_get_shareable_buffer_ids (ezv_ctx_t ctx, int buffer_ids[])
{
  if (ctx->class->get_shareable_buffer_ids != NULL)
    ...
    exit_with_error ("Buffer sharing not supported on %s ctx",
                     ezv_ctx_typestr (ctx));
}
...
int ezv_do_1D_picking (ezv_ctx_t ctx, int mousex, int mousey)
{
  if (ctx->class->do_1D_picking != NULL)
    ...
    exit_with_error ("1D picking not supported on %s ctx",
                     ezv_ctx_typestr (ctx));
}
...
void ezv_do_2D_picking (ezv_ctx_t ctx, int mousex, int mousey, int *x, int *y)
{
  if (ctx->class->do_2D_picking != NULL)
    ...
    exit_with_error ("2D picking not supported on %s ctx",
                     ezv_ctx_typestr (ctx));
}
...
unsigned ezv_get_linepitch (ezv_ctx_t ctx)
{
  if (ctx->class->get_linepitch != NULL)
    ...
    exit_with_error ("get_linepitch not supported on %s ctx",
                     ezv_ctx_typestr (ctx));
}
...
{
  if (ctx->class->cellmask_reset != NULL)
    ctx->class->cellmask_reset (ctx);
  else
    exit_with_error ("cellmask_reset not supported on %s ctx",
                     ezv_ctx_typestr (ctx));
}
...
{
  if (ctx->class->cellmask_set1D_hidden != NULL)
    ctx->class->cellmask_set1D_hidden (ctx, first_cell, nb_cells, mask);
  else
    exit_with_error ("cellmask_set1D_hidden not supported on %s ctx",
                     ezv_ctx_typestr (ctx));
}

void ezv_cellmask_set1D_alpha (ezv_ctx_t ctx, uint8_t alpha,
                               unsigned first_cell, unsigned nb_cells, uint8_t *mask)
{
  if (ctx->class->cellmask_set1D_alpha != NULL)
    ctx->class->cellmask_set1D_alpha (ctx, alpha, first_cell, nb_cells, mask);
  else
    exit_with_error ("cellmask_set1D_alpha not supported on %s ctx",
                     ezv_ctx_typestr (ctx));
}