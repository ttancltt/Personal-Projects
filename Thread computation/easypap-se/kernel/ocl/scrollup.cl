...
__kernel void scrollup_ocl_ouf (__global unsigned *ina, __global unsigned *inb,
                                __global unsigned *out, __global unsigned *mask,
                                unsigned framecolor)
{
  unsigned y       = get_global_id (1);
  unsigned x       = get_global_id (0);
  ...
  out [y * DIM + x] =
      float4_to_color (color_a * ratio + color_b * (1.0f - ratio));
}
