...
static void *thread_block (void *arg){
 unsigned me =(size_t)(void *)arg;
 unsigned y = me*(DIM/nb_threads);
 unsigned h = DIM/nb_threads;
 int r;
 if ((r = DIM % nb_threads) > 0){
  h += r;
 }
  do_tile (0, y, DIM, h, me);

 return NULL;
}

unsigned mandel_compute_thread_block (unsigned nb_iter){
  ...
  for (unsigned it = 1; it <= nb_iter; it++){
    ...
    for(int i = 0; i< nb_threads; i++)
      pthread_create(&pid[i], NULL, thread_block,(void *)(size_t) i);
...
static void *thread_cyclic (void *arg){
 unsigned me =(ssize_t)(void *)arg;

 
  for (int i = me; i < DIM ; i += nb_threads)
    do_tile (0, i, DIM, 1, me);
...
unsigned mandel_compute_thread_cyclic (unsigned nb_iter){
  ...
  for (unsigned it = 1; it <= nb_iter; it++){
    ...
    for(int i = 0; i< nb_threads; i++)
      pthread_create(&pid[i], NULL, thread_cyclic,(void *)(ssize_t) i);
...
static void *thread_dyn (void *arg){
  unsigned me =(ssize_t)(void *)arg;
  int p;
  
  while ((p=distrib_get()) !=-1)
  {
    do_tile (0, p, DIM, 1, me);
  }
...
unsigned mandel_compute_thread_dyn (unsigned nb_iter){
  ...
  for (unsigned it = 1; it <= nb_iter; it++){
    ...
    for(int i = 0; i< nb_threads; i++)
      pthread_create(&pid[i], NULL, thread_dyn,(void *)(ssize_t) i);
