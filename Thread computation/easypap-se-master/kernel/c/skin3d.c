...
int skin3d_do_patch_default (int start_cell, int end_cell)
{
  for (int c = start_cell; c < end_cell; c++) {
    // We use double buffering, so next_data (c) is the value of cell c for the
    // next iteration, while cur_data (c) is the value of cell c for the current
    // iteration.

    // For now, do nothing: just copy current value to next value, so that the
    // visualization can show the initial configuration
    next_data (c) = cur_data (c);
  }
...
unsigned skin3d_compute_seq (unsigned nb_iter)
{
  ...
  // Computation stops after 1 iteration
  // (return 0 would mean "continue iterating")
  return 1;
}
...
static void draw_coord (int coord)
{
  ...
  for (int c = 0; c < NB_CELLS; c++) {
    ...
    cur_data (c) = (center - min_val) / (max_val - min_val);
  }
}
...
void skin3d_draw (char *param)
{
  ...
  hooks_draw_helper (param, skin3d_draw_uni);
}
