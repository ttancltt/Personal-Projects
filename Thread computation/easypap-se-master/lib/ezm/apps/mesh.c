...
static int process_events (int blocking)
{
  SDL_Event event;
  int cont = 1;
  int r;
    ...
    if (r > 0) {
      ...
      case SDL_KEYDOWN:
        ...
        case SDLK_d:
          ...
          break;
        default:
          cont = ezv_process_event (ctx, nb_ctx, &event, NULL, NULL);
        }
        break;
      default:
        cont = ezv_process_event (ctx, nb_ctx, &event, NULL, NULL);
...
int do_patch (int p)
{
  ...
  ezm_start_task (recorder, me);

  int change =
      do_one_patch (mesh.patch_first_cell [p], mesh.patch_first_cell [p + 1]);

  ezm_end_task_1D (recorder, me, mesh.patch_first_cell [p],
...
int main (int argc, char *argv [])
{
  ...
  if (!do_trace) {
    ...
    // ctx #1
    ctx [nb_ctx++] =
        ezv_ctx_create (EZV_CTX_TYPE_MESH3D, "Black & White", WIN_WIDTH, 0,
                        WIN_WIDTH / 2, WIN_HEIGHT / 2, EZV_ENABLE_CLIPPING);
    // Attach mesh
    ezv_mesh3d_set_mesh (ctx [1], &mesh);

    ezv_use_data_colors_predefined (ctx [1], EZV_PALETTE_BW);
#endif
  }

  // Profiling
  recorder = ezm_recorder_create (nb_openmp_threads (), 0, NULL);
  if (do_trace) {
    ezm_recorder_attach_tracerec (recorder, "data/traces/max.evt",
                                  "CC Labelling (OpenMP)");
    ezm_recorder_store_mesh3d_filename (recorder, mesh_file);
  } else {
    ezm_set_cpu_palette (recorder, EZV_PALETTE_RAINBOW, 0);
    ezm_helper_add_perfmeter (recorder, ctx, &nb_ctx, EZV_MONITOR_DEFAULT,
                              NULL);
    ezm_helper_add_footprint (recorder, ctx, &nb_ctx, 0);
  }
...
#ifdef EXTRA_WINDOW
        ezv_set_data_colors (ctx [1], mesh_values);
#endif
      }
    }
