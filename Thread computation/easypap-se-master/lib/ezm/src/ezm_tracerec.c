...
static int cpu_to_lane (ezm_tracerec_t rec, int thr, task_type_t task_type)
{
  if (thr < rec->nb_thr)
    return thr;

  return rec->nb_thr + 2 * (thr - rec->nb_thr) +
         (task_type == TASK_TYPE_COMPUTE ? 0 : 1);
}

ezm_tracerec_t ezm_tracerec_create (unsigned nb_thr, unsigned nb_gpus,
                                    char *file, char *label)
{
  ...
  rec->pu_stat = malloc ((nb_thr + 2 * nb_gpus) * sizeof (*rec->pu_stat));
  ...
  FUT_DO_PROBE2 (TRACE_NB_THREADS, nb_thr, nb_gpus * 2);
  if (label != NULL)
...
void ezm_tracerec_store_topo (ezm_tracerec_t rec, const char *topo_filename)
{
  if (topo_filename != NULL)
    FUT_DO_PROBESTR (TRACE_TOPOFILE, topo_filename);
}

void ezm_tracerec_store_thr_bindings (ezm_tracerec_t rec,
                                      ezv_topo_pu_block_t *bindings)
{
  if (bindings != NULL) {
    FUT_DO_PROBE0 (TRACE_BINDING_HEADER);
    for (unsigned i = 0; i < rec->nb_thr; i++)
      FUT_DO_PROBE3 (TRACE_THR_BINDING, i, bindings [i].first_pu, bindings [i].nb_pus);
  } else
    exit_with_error ("ezm_tracerec_store_thr_bindings: bindings is NULL");
}
