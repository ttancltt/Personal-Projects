...
static void usage (char *progname, int val)
{
  ...
  fprintf (stderr,
           "options can be:\n"
           "\t-a\t| --align\t\t: align iterations\n"
           "\t-c\t| --compare\t\t: compare last two traces\n"
           "\t-d\t| --dir <dir>\t\t: specify trace directory\n"
           "\t-h\t| --help\t\t: display help\n"
           "\t-i\t| --iteration <i>\t: display iteration i\n"
           "\t-p\t| --params\t\t: use options from params.txt file\n"
           "\t-b\t| --brightness <n>\t: set brightness level (0-255)\n"
           "\t-r\t| --range <i> <j>\t: display iteration range [i-j]\n"
           "\t-t\t| --thread\t\t: disable topology display (old thread mode)\n"
           "\t-w\t| --whole-trace\t\t: display all iterations\n"
           "\t\t  --which\t\t: display easyview executable path\n");
...
static char **filter_args (int *argc, char *argv [])
{
  ...
  while (*argc > 0) {
    if (!strcmp (*argv, "--align") || !strcmp (*argv, "-a")) {
      ...
      whole_trace = 1;
    } else if (!strcmp (*argv, "--thread") || !strcmp (*argv, "-t")) {
      ...
      // unknown option
      exit_with_error ("Error: unknown option '%s'\n", *argv);
      usage (progname, 1);
    } else {
      break;
    }
...
int main (int argc, char **argv)
{
  ...
  if (whole_trace)
    ...
    trace_view_set (first_iteration, last_iteration);
