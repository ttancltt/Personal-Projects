...
#define CELLMASK_HIDDEN 256U
#define CELLMASK_ALPHA 512U
...
typedef struct render_ctx_s
{
  ...
  GLuint dapu_colors_loc, dapu_info_loc, dapu_values_loc, dapu_palettesize_loc,
      dapu_brightness_loc;
  GLuint picking_info_loc;
  GLuint cut_info_loc;
  uint32_t *cellmask;
#ifdef __linux__
  GLuint MASK_SSBO;
  GLuint filter_shader;
  GLuint filter_info_loc;
  unsigned filter_need_refresh;
#else
  GLuint TBO_MASK; // Texture Buffer Object containing per-cell masking info
  GLuint tex_mask;    // GL Texture associated to TBO_MASK
  GLuint cpu_cellmask_loc, data_cellmask_loc, dapu_cellmask_loc,
      picking_cellmask_loc, cut_cellmask_loc;
#endif
} mesh3d_render_ctx_t;
...
void mesh3d_renderer_zplane_update (ezv_ctx_t ctx[], unsigned nb_ctx, float dz)
{
  ...
  mesh3d_renderer_mvp_update (ctx, nb_ctx, 0.0f, 0.0f, 0.0f);
#ifdef __linux__
  mesh3d_render_ctx_t *renctx = ezv_mesh3d_renderer (ctx[0]);
  renctx->filter_need_refresh = 1;
#endif
}
...
static void mesh3d_zplane_init (ezv_ctx_t ctx)
{
  ...
  if (ctx->cellmask_enabled)
#ifdef __linux__
    renctx->cut_shader = ezv_shader_create (
        "mesh3d/cut.vs", "mesh3d/cut-mask-ssbo.gs", "mesh3d/cut.fs");
#else
    renctx->cut_shader = ezv_shader_create (
        "mesh3d/cut.vs", "mesh3d/cut-mask.gs", "mesh3d/cut.fs");
#endif
  else
    renctx->cut_shader =
  ...
  if (ctx->cellmask_enabled)
#ifdef __linux__
    ezv_shader_bind_ssbo (renctx->cut_shader, "CellBuffer",
                          BINDING_POINT_USERMASK);
#else
    ezv_shader_get_uniform_loc (renctx->cut_shader, "CellMask",
                                &renctx->cut_cellmask_loc);
#endif
  ...
  if (mesh->mesh_type == MESH3D_TYPE_VOLUME) {
    renctx->filter_shader = ezv_shader_create (
        "mesh3d/cut.vs", "mesh3d/filter.gs", "mesh3d/cut.fs");

    ezv_shader_bind_uniform_buf (renctx->filter_shader, "Matrices",
                                 BINDING_POINT_MATRICES);
    ezv_shader_bind_uniform_buf (renctx->filter_shader, "Clipping",
                                 BINDING_POINT_CLIPPING);
    ezv_shader_bind_uniform_buf (renctx->filter_shader, "CustomColors",
                                 BINDING_POINT_CUSTOM_COLORS);

    ezv_shader_get_uniform_loc (renctx->filter_shader, "TriangleInfo",
                                &renctx->filter_info_loc);
    ezv_shader_bind_ssbo (renctx->filter_shader, "MaskBuffer",
                          BINDING_POINT_USERMASK);
...
static void mesh3d_zplane_precompute_mask (ezv_ctx_t ctx)
{
  mesh3d_render_ctx_t *renctx = ezv_mesh3d_renderer (ctx);
  mesh3d_obj_t *mesh          = ezv_mesh3d_mesh (ctx);

  if (mesh->mesh_type != MESH3D_TYPE_VOLUME || !renctx->filter_need_refresh)
    return;
  
  glBindBuffer (GL_SHADER_STORAGE_BUFFER, renctx->MASK_SSBO);
  glBufferSubData (GL_SHADER_STORAGE_BUFFER, 0,
                   mesh->nb_cells * sizeof (uint32_t), renctx->cellmask);

  if (!ctx->clipping_enabled || !ctx->clipping_active)
    return;

  // Run shader
  glUseProgram (renctx->filter_shader);
  glDrawElements (GL_TRIANGLES, mesh->nb_triangles * 3, GL_UNSIGNED_INT, 0);

  renctx->filter_need_refresh = 0;
}
...
static void mesh3d_picking_init (ezv_ctx_t ctx)
{
  ...
  if (ctx->cellmask_enabled)
  // use 'mask'-enabled shaders
#ifdef __linux__
    renctx->picking_shader =
        ezv_shader_create ("mesh3d/picking.vs", "mesh3d/picking-mask-ssbo.gs",
                           "mesh3d/picking-mask.fs");
#else
    renctx->picking_shader =
        ezv_shader_create ("mesh3d/picking.vs", "mesh3d/picking-mask.gs",
                           "mesh3d/picking-mask.fs");
#endif
  else
    // use 'normal' shaders
    renctx->picking_shader =
        ezv_shader_create ("mesh3d/picking.vs", NULL, "mesh3d/picking.fs");

  ezv_shader_bind_uniform_buf (renctx->picking_shader, "Matrices",
                               BINDING_POINT_MATRICES);

  if (ctx->cellmask_enabled) {
    ezv_shader_get_uniform_loc (renctx->picking_shader, "TriangleInfo",
                                &renctx->picking_info_loc);
#ifdef __linux__
    ezv_shader_bind_ssbo (renctx->picking_shader, "CellBuffer",
                          BINDING_POINT_USERMASK);
#else
    ezv_shader_get_uniform_loc (renctx->picking_shader, "CellMask",
                                &renctx->picking_cellmask_loc);
#endif
  }
  ...
  glActiveTexture (GL_TEXTURE0 + EZV_PICK_TEXTURE_NUM);
...
static void mesh3d_build_shaders (ezv_ctx_t ctx)
{
  mesh3d_render_ctx_t *renctx = ezv_mesh3d_renderer (ctx);
  ...
  if (ctx->cellmask_enabled) {
    // use 'mask'-enabled shaders
#ifdef __linux__
    renctx->cpu_shader = ezv_shader_create (
        "mesh3d/generic.vs", "mesh3d/cpu-mask-ssbo.gs", "mesh3d/generic.fs");
    renctx->data_shader = ezv_shader_create (
        "mesh3d/generic.vs", "mesh3d/data-mask-ssbo.gs", "mesh3d/generic.fs");
    renctx->dapu_shader =
        ezv_shader_create ("mesh3d/generic.vs", "mesh3d/cpu_data-mask-ssbo.gs",
                           "mesh3d/generic.fs");
#else
    renctx->cpu_shader = ezv_shader_create (
        "mesh3d/generic.vs", "mesh3d/cpu-mask.gs", "mesh3d/generic.fs");
    renctx->data_shader = ezv_shader_create (
        "mesh3d/generic.vs", "mesh3d/data-mask.gs", "mesh3d/generic.fs");
    renctx->dapu_shader = ezv_shader_create (
        "mesh3d/generic.vs", "mesh3d/cpu_data-mask.gs", "mesh3d/generic.fs");
#endif
  } else {
    renctx->cpu_shader = ezv_shader_create (
        "mesh3d/generic.vs", "mesh3d/cpu.gs", "mesh3d/generic.fs");
    renctx->data_shader = ezv_shader_create (
...
static void mesh3d_configure_uniforms (ezv_ctx_t ctx)
{
  mesh3d_render_ctx_t *renctx = ezv_mesh3d_renderer (ctx);
  ...
  if (ctx->cellmask_enabled) {
#ifdef __linux__
    ezv_shader_bind_ssbo (renctx->cpu_shader, "CellBuffer",
                          BINDING_POINT_USERMASK);
    ezv_shader_bind_ssbo (renctx->data_shader, "CellBuffer",
                          BINDING_POINT_USERMASK);
    ezv_shader_bind_ssbo (renctx->dapu_shader, "CellBuffer",
                          BINDING_POINT_USERMASK);
#else
    ezv_shader_get_uniform_loc (renctx->cpu_shader, "CellMask",
                                &renctx->cpu_cellmask_loc);
    ezv_shader_get_uniform_loc (renctx->data_shader, "CellMask",
                                &renctx->data_cellmask_loc);
    ezv_shader_get_uniform_loc (renctx->dapu_shader, "CellMask",
                                &renctx->dapu_cellmask_loc);
#endif
  }
}

static void init_shaders_and_uniforms (ezv_ctx_t ctx)
{
  // compile shaders and build program
  mesh3d_build_shaders (ctx);

  // Configure uniforms
  mesh3d_configure_uniforms (ctx);
...
void mesh3d_renderer_init (ezv_ctx_t ctx)
{
  ...
  // Allocate render_ctx
  mesh3d_render_ctx_t *renctx = calloc (1, sizeof (mesh3d_render_ctx_t));

  ezv_mesh3d_set_renderer (ctx, renctx);
}

void mesh3d_renderer_finalize (ezv_ctx_t ctx)
{
  mesh3d_render_ctx_t *renctx = ezv_mesh3d_renderer (ctx);

  glDeleteBuffers (1, &renctx->UBO_MAT);
  glDeleteBuffers (1, &renctx->UBO_CUST);
  glDeleteBuffers (1, &renctx->UBO_CLIP);

  if (ctx->picking_enabled) {
    glDeleteTextures (1, &renctx->pickingTexture);
    glDeleteTextures (1, &renctx->depthTexture);
    glDeleteFramebuffers (1, &renctx->FBO);

    glDeleteProgram (renctx->picking_shader);
  }

  if (ctx->clipping_enabled) {
    glDeleteVertexArrays (1, &renctx->VAO_ZPLANE);
    glDeleteBuffers (1, &renctx->VBO_ZPLANE);
    glDeleteBuffers (1, &renctx->EBO_ZPLANE);

    glDeleteProgram (renctx->clipping_shader);
    glDeleteProgram (renctx->cut_shader);
  }

  // Allocated in set_mesh
  glDeleteVertexArrays (1, &renctx->VAO);
  glDeleteBuffers (1, &renctx->VBO);
  glDeleteBuffers (1, &renctx->VBO_IND);
  glDeleteBuffers (1, &renctx->TBO_INFO);
  glDeleteTextures (1, &renctx->tex_tinfo);

  if (ezv_palette_is_defined (&ctx->data_palette)) {
    glDeleteBuffers (1, &renctx->UBO_DATACOL);
    glDeleteBuffers (1, &renctx->TBO_DATACOL);
    glDeleteTextures (1, &renctx->tex_datacol);
  }
  ...
  if (ctx->cellmask_enabled) {
    free (renctx->cellmask);
    renctx->cellmask = NULL;

#ifdef __linux__
    glDeleteBuffers (1, &renctx->MASK_SSBO);

    mesh3d_obj_t *mesh = ezv_mesh3d_mesh (ctx);

    if (ctx->clipping_enabled && mesh->mesh_type == MESH3D_TYPE_VOLUME) {
      glDeleteProgram (renctx->filter_shader);
    }
#else
    glDeleteBuffers (1, &renctx->TBO_MASK);
    glDeleteTextures (1, &renctx->tex_mask);
#endif
  }
  ...
  glDeleteProgram (renctx->dapu_shader);

  free (renctx);
  ezv_mesh3d_set_renderer (ctx, NULL);
}
...
void mesh3d_renderer_set_mesh (ezv_ctx_t ctx)
{
  ...
  if (ctx->clipping_enabled) {
    glProgramUniform1i (renctx->cut_shader, renctx->cut_info_loc,
                        EZV_INFO_TEXTURE_NUM);
#ifdef __linux__
    glProgramUniform1i (renctx->filter_shader, renctx->filter_info_loc,
                        EZV_INFO_TEXTURE_NUM);
#endif
  }
  ...
  if (ctx->cellmask_enabled) {
    ...
    if (ctx->picking_enabled)
      glProgramUniform1i (renctx->picking_shader, renctx->picking_info_loc,
                          EZV_INFO_TEXTURE_NUM);

    renctx->cellmask = calloc (mesh->nb_cells, sizeof (uint32_t));

#ifdef __linux__
    // We use a Shader Storage Buffer Object for CellMask on Linux
    glGenBuffers (1, &renctx->MASK_SSBO);
    glBindBuffer (GL_SHADER_STORAGE_BUFFER, renctx->MASK_SSBO);
    glBufferData (GL_SHADER_STORAGE_BUFFER, mesh->nb_cells * sizeof (uint32_t),
                  NULL, GL_DYNAMIC_COPY);
    glBindBufferBase (GL_SHADER_STORAGE_BUFFER, BINDING_POINT_USERMASK,
                      renctx->MASK_SSBO);
#else
    // Texture Buffer Object containing per-cell masking info
    glGenBuffers (1, &renctx->TBO_MASK);
    glBindBuffer (GL_TEXTURE_BUFFER, renctx->TBO_MASK);
    glBufferData (GL_TEXTURE_BUFFER, mesh->nb_cells * sizeof (int), NULL,
                  GL_DYNAMIC_DRAW); // To be refreshed regularly
    glActiveTexture (GL_TEXTURE0 + EZV_MASK_TEXTURE_NUM);
    glGenTextures (1, &renctx->tex_mask);
    glBindTexture (GL_TEXTURE_BUFFER, renctx->tex_mask);
    glTexBuffer (GL_TEXTURE_BUFFER, GL_R32I, renctx->TBO_MASK);

    // bind uniform buffer object to texture #4
    glProgramUniform1i (renctx->cpu_shader, renctx->cpu_cellmask_loc,
                        EZV_MASK_TEXTURE_NUM);
    glProgramUniform1i (renctx->data_shader, renctx->data_cellmask_loc,
                        EZV_MASK_TEXTURE_NUM);
    glProgramUniform1i (renctx->dapu_shader, renctx->dapu_cellmask_loc,
                        EZV_MASK_TEXTURE_NUM);
    if (ctx->picking_enabled)
      glProgramUniform1i (renctx->picking_shader, renctx->picking_cellmask_loc,
                          EZV_MASK_TEXTURE_NUM);
    if (ctx->clipping_enabled)
      glProgramUniform1i (renctx->cut_shader, renctx->cut_cellmask_loc,
                          EZV_MASK_TEXTURE_NUM);
#endif
  }
}
...
static void update_cellmask (ezv_ctx_t ctx, unsigned first_cell,
                             unsigned nb_cells)
{
  mesh3d_render_ctx_t *renctx = ezv_mesh3d_renderer (ctx);
...
#else
  glBindBuffer (GL_TEXTURE_BUFFER, renctx->TBO_MASK);
  glBufferSubData (GL_TEXTURE_BUFFER, first_cell * sizeof (uint32_t),
                   nb_cells * sizeof (uint32_t), renctx->cellmask + first_cell);
#endif
}

void mesh3d_cellmask_reset (ezv_ctx_t ctx)
{
  if (ctx->cellmask_enabled == 0)
    return;

  mesh3d_render_ctx_t *renctx = ezv_mesh3d_renderer (ctx);
  mesh3d_obj_t *mesh          = ezv_mesh3d_mesh (ctx);

  memset (renctx->cellmask, 0, mesh->nb_cells * sizeof (uint32_t));

  update_cellmask (ctx, 0, mesh->nb_cells);
}

void mesh3d_cellmask_set_hidden (ezv_ctx_t ctx, unsigned first_cell,
                                 unsigned nb_cells, uint8_t *mask)
{
  ...
  if (mask == NULL)
    for (unsigned c = first_cell; c < first_cell + nb_cells; c++)
      renctx->cellmask[c] = CELLMASK_HIDDEN;
  else
    for (unsigned c = first_cell; c < first_cell + nb_cells; c++)
      if (mask[c])
        renctx->cellmask[c] = CELLMASK_HIDDEN;

  update_cellmask (ctx, first_cell, nb_cells);
}

void mesh3d_cellmask_set_alpha (ezv_ctx_t ctx, uint8_t alpha,
                                ...
                                uint8_t *mask)
{
  ...
  uint32_t value = CELLMASK_ALPHA | alpha;
  if (mask == NULL)
    for (unsigned c = first_cell; c < first_cell + nb_cells; c++)
      renctx->cellmask[c] = value;
  else
    for (unsigned c = first_cell; c < first_cell + nb_cells; c++)
      if (mask[c])
        renctx->cellmask[c] = value;
...
#ifdef __linux__
    mesh3d_zplane_precompute_mask (ctx);
#endif
                          ...
                          unsigned in)
{
  if (shift_mod) {
    ...
    if(ctx[0]->clipping_enabled && mesh->mesh_type == MESH3D_TYPE_VOLUME)
      renctx->filter_need_refresh = 1;
#endif
