...
static void load_texture_once (void)
{
  ...
  snprintf (file, sizeof(file), "%s/share/ezv/img/ascii-black.png", ezv_prefix);
#else
...
void ezv_hud_finalize (ezv_ctx_t ctx)
{
  hud_ctx_t *hudctx = ctx->hud_ctx;

  glDeleteBuffers (1, &hudctx->UBO_HUD);
  glDeleteBuffers (1, &hudctx->VBO_HUD);
  glDeleteVertexArrays (1, &hudctx->VAO_HUD);
  glDeleteBuffers (1, &hudctx->EBO_HUD);
  glDeleteVertexArrays (1, &hudctx->VAO_BACKG);
  glDeleteBuffers (1, &hudctx->VBO_BACKG);

  glDeleteTextures (1, &hudctx->digitTexture);

  glDeleteProgram (hudctx->hud_shader);
  glDeleteProgram (hudctx->backg_shader);
