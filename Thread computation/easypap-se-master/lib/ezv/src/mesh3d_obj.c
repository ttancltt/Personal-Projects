...
void mesh3d_obj_delete (mesh3d_obj_t *mesh)
{
  ...
  if (mesh->index_first_neighbor != NULL)
  ...
  if (mesh->metap_first_border_patch != NULL)
    free (mesh->metap_first_border_patch);
  ...
  for (int z = 0; z < nbz; z++) {
    for (int y = 0; y < nby; y++) {
      for (int x = 0; x < nbx; x++) {
        ...
        if (z > 0) {
          mesh->triangle_info [nbt]     = ISINNER;
          mesh->triangle_info [nbt + 1] = ISINNER;
        }
        ...
        mesh->triangle_info [nbt] |= EDGE1;
        mesh->triangle_info [nbt + 1] |= EDGE2;
        add_triangle (mesh, v [0], v [1], v [4]); // bottom
        add_triangle (mesh, v [1], v [5], v [4]);
        ...
        mesh->triangle_info [nbt] |= EDGE1;
        mesh->triangle_info [nbt + 1] |= EDGE2;
        add_triangle (mesh, v [5], v [1], v [7]); // right
        add_triangle (mesh, v [1], v [3], v [7]);
        ...
        mesh->triangle_info [nbt] |= EDGE1;
        mesh->triangle_info [nbt + 1] |= EDGE2;
        add_triangle (mesh, v [4], v [5], v [6]); // back
        add_triangle (mesh, v [5], v [7], v [6]);
      }
    }
  }
  ...
  for (int z = 0; z < nbz; z++) {
    for (int y = 0; y < nby; y++) {
      for (int x = 0; x < nbx; x++) {
        ...
        v [7] =
            (z + 1) * enveloppe + ((y + 1) % nby) * circle + ((x + 1) % nbx);
        ...
        mesh->triangle_info [nbt]     = ISINNER | EDGE1;
        mesh->triangle_info [nbt + 1] = ISINNER | EDGE0;
        add_triangle (mesh, v [4], v [6], v [0]); // left
        add_triangle (mesh, v [0], v [6], v [2]);
        ...
        mesh->triangle_info [nbt]     = ISINNER | EDGE1;
        mesh->triangle_info [nbt + 1] = ISINNER | EDGE2;
        add_triangle (mesh, v [0], v [1], v [4]); // bottom
        add_triangle (mesh, v [1], v [5], v [4]);
...
static void build_neighbors_cyl (mesh3d_obj_t *mesh, unsigned size_x,
                                unsigned size_y)
{
  int index = 0;

  mesh->max_neighbors = 4;
  mesh->total_neighbors = mesh->nb_cells * mesh->max_neighbors - 2 * size_x; // except for the 2*size_x cells on the two borders
  ...
  for (int c = 0; c < mesh->nb_cells; c++) {
    ...
    // down
    if (y > 0)
      mesh->neighbors [index++] = (y - 1) * size_x + x;
    // left
    mesh->neighbors [index++] = y * size_x + (x + size_x - 1) % size_x;
    // right
    mesh->neighbors [index++] = y * size_x + (x + size_x + 1) % size_x;
    // up
    if (y < size_y - 1)
  ...
  if (mesh->total_neighbors != index)
    exit_with_error (
        "mesh3d: the final number of neighbors (%d) does not match "
        "the initial one (%d)",
        index, mesh->total_neighbors);
}
                                       ...
                                       unsigned size_y)
{
  ...
  build_neighbors_cyl (mesh, size_x, size_y);
}
...
static unsigned triangles_common_edges (mesh3d_obj_t *mesh, int t1, int t2)
{
  ...
  for (unsigned i1 = 0; i1 < 3; i1++) {
    int v = mesh->triangles [3 * t1 + i1];
    if (mesh->triangles [3 * t2 + 0] == v ||
        mesh->triangles [3 * t2 + 1] == v || mesh->triangles [3 * t2 + 2] == v)
      vmatch |= (1 << i1);
  }
...
static void triangle_set_inner (mesh3d_obj_t *mesh, int t)
{
  if ((mesh->triangle_info [t] & EDGE0) || (mesh->triangle_info [t] & EDGE1) ||
      (mesh->triangle_info [t] & EDGE2))
    mesh->triangle_info [t] |= ISINNER;
}

static unsigned triangles_are_same (mesh3d_obj_t *mesh, int t1, int t2)
{
  for (unsigned e = 0; e < 3; e++) {
    int v = mesh->triangles [3 * t1 + e];
    if (mesh->triangles [3 * t2 + 0] != v &&
        mesh->triangles [3 * t2 + 1] != v &&
        mesh->triangles [3 * t2 + 2] != v) {
      // Vertex not found in t2, impossible to match
      return 0;
    }
  }

  return 1; // all vertices match
}

static void mark_common_triangles_as_inner (mesh3d_obj_t *mesh)
{
  for (int c1 = 0; c1 < mesh->nb_cells; c1++) {
    // for each neighbor cell
    for (int i = mesh->index_first_neighbor [c1];
         i < mesh->index_first_neighbor [c1 + 1]; i++) {
      int c2 = mesh->neighbors [i];
      // find all triangles shared by c1 and c2
      for (int t1 = mesh->cells [c1]; t1 < mesh->cells [c1 + 1]; t1++) {
        for (int t2 = mesh->cells [c2]; t2 < mesh->cells [c2 + 1]; t2++) {
          // check if t1 and t2 are the same vertices list in potentially
          // different order
          if (triangles_are_same (mesh, t1, t2)) {
            // mark triangles as inner
            triangle_set_inner (mesh, t1);
            triangle_set_inner (mesh, t2);
          }
        }
      }
    }
...
static int copy_without_duplicates (mesh3d_obj_t *mesh, float *src, int *vindex)
{
  ...
  for (int v = 0; v < mesh->nb_vertices; v++) {
    ...
    for (o = 0; o < index; o++)
      if (mesh->vertices [3 * o + 0] == c0 &&
          mesh->vertices [3 * o + 1] == c1 && mesh->vertices [3 * o + 2] == c2)
...
static void load_obj_file (const char *filename, mesh3d_obj_t *mesh)
{
  ...
  for (int i = 0; i < attrib.num_face_num_verts; i++) {
    if (attrib.face_num_verts [i] == 3) {
      ...
      // bottom
      mesh->triangle_info [nbt] |= (EDGE1 | ((nbc - 1) << CELLNO_SHIFT));
      add_triangle (mesh, idx [0], idx [1], idx [4]);
      mesh->triangle_info [nbt] |= (EDGE2 | ((nbc - 1) << CELLNO_SHIFT));
      add_triangle (mesh, idx [1], idx [5], idx [4]);

      // right
      mesh->triangle_info [nbt] |= (EDGE1 | ((nbc - 1) << CELLNO_SHIFT));
      add_triangle (mesh, idx [5], idx [1], idx [7]);
      mesh->triangle_info [nbt] |= (EDGE2 | ((nbc - 1) << CELLNO_SHIFT));
      add_triangle (mesh, idx [1], idx [3], idx [7]);

      // back
      mesh->triangle_info [nbt] |= (EDGE1 | ((nbc - 1) << CELLNO_SHIFT));
      add_triangle (mesh, idx [4], idx [5], idx [6]);
      mesh->triangle_info [nbt] |= (EDGE2 | ((nbc - 1) << CELLNO_SHIFT));
      add_triangle (mesh, idx [5], idx [7], idx [6]);
  ...
  if (attrib.num_neighbors > 0) {
    ...
    mesh->min_neighbors = TINYOBJ_MAX_NEIGHBORS;
    ...
    for (int c = 0; c < mesh->nb_cells; c++) {
      ...
      for (int k = 0; k < attrib.connectivity_size [c]; k++)
        mesh->neighbors [index++] =
            attrib.connectivity [TINYOBJ_MAX_NEIGHBORS * c + k];
    }
    mesh->index_first_neighbor [mesh->nb_cells] = index;
    mesh->total_neighbors                       = index;
    ...
    if (mesh->mesh_type == MESH3D_TYPE_VOLUME)
      mark_common_triangles_as_inner (mesh);
  }
...
void mesh3d_obj_load (const char *filename, mesh3d_obj_t *mesh)
{
  ...
  else if (!strcmp (filename, "cube.cgns"))
    mesh3d_obj_build_cube_volume (mesh, 64);
                       ...
                       int with_patches)
{
  ...
  for (int c = 0; c < mesh->nb_cells; c++) {
    ...
    if (nb_tri == 1) {
      ...
      // To store v0, v1 … v7, we use front0[0 2 1], front1[2], back0[0 1 2] &
      // back1[1]
      fprintf (f, "f %d %d %d %d %d %d %d %d\n",
               ...
               mesh->triangles [3 * (mesh->cells [c] + 10) + 1] + 1,
...
void mesh3d_reorder_cells (mesh3d_obj_t *mesh, int newpos [])
{
...
void mesh3d_reorder_partitions (mesh3d_obj_t *mesh, int newpos [])
{
  ...
  for (int p = 0; p < mesh->nb_patches; p++) {
    ...
    first_cell [p] = index;
    for (int c = mesh->patch_first_cell [op];
         c < mesh->patch_first_cell [op + 1]; c++)
      cell_npos [c] = index++;
...
void mesh3d_obj_partition (mesh3d_obj_t *mesh, unsigned nbpart, int flag)
{
  ...
  if (flag & MESH3D_PART_SHOW_FRONTIERS) {
    ...
    for (int p = 0; p < mesh->nb_patches; p++)
      for (int c = mesh->patch_first_cell [p];
           c < mesh->patch_first_cell [p + 1]; c++)
        parttab [c] = p;
