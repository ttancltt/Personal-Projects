...
static void ezv_load_opengl (unsigned verbose)
{
  static int done = 0;
                          ...
                          int y, int w, int h, int flags)
{
...
#ifdef __linux__
  // Workaround to a weird SDL behavior that sometimes changes the main thread's cpuset
  // So we save the cpuset before calling SDL_CreateWindow, and restore it afterwards
  hwloc_cpuset_t myset = hwloc_bitmap_alloc ();
  ezv_topo_get_current_cpuset (myset);
#endif
  ...
  ctx->clipping_enabled =
      ((flags & EZV_ENABLE_CLIPPING) && (ctx_type == EZV_CTX_TYPE_MESH3D)) ? 1
                                                                           : 0;
  ctx->cellmask_enabled = (flags & EZV_ENABLE_CELLMASK) ? 1 : 0;
#ifdef __linux__
  // We force usage of cellmask when clipping is enabled on Linux
  ctx->cellmask_enabled |= ctx->clipping_enabled;
#endif
  ...
  ezv_load_opengl ((flags & EZV_VERBOSE) ? 1 : 0);
...
#ifdef __linux__
  ezv_topo_set_current_cpuset (myset);
...
{
  for (unsigned c = 0; c < nb_ctx; c++) {
    if (ctx [c]->hud_enabled)
      ezv_hud_finalize (ctx [c]);

    switch (ctx [c]->type) {
    case EZV_CTX_TYPE_IMG2D: {
      // Initialize img2d specific data
      ezv_img2d_object_delete (ctx [c]);
      break;
    }
    case EZV_CTX_TYPE_MESH3D: {
      // Initialize mesh3d specific data
      ezv_mesh3d_object_delete (ctx [c]);
      break;
    }
    case EZV_CTX_TYPE_MONITOR: {
      // Initialize mon specific data
      ezv_mon_object_delete (ctx [c]);
      break;
    }
    default:
      exit_with_error ("ctx_type %d not supported", ctx [c]->type);
    }

    ezv_palette_free (&ctx [c]->cpu_palette);
    ezv_palette_free (&ctx [c]->data_palette);

    if (ctx [c]->cpu_colors != NULL)
      free (ctx [c]->cpu_colors);

    SDL_GL_DeleteContext (ctx [c]->glcontext);
    SDL_DestroyWindow (ctx [c]->win);
