...
void img2d_renderer_finalize (ezv_ctx_t ctx)
{
  img2d_render_ctx_t *renctx = ezv_img2d_renderer (ctx);
  ...
  if (ezv_palette_is_defined (&ctx->cpu_palette))
    glDeleteTextures (1, &renctx->cpuTexture);
  ...
  free (renctx);
  ezv_img2d_set_renderer (ctx, NULL);
}
...
void img2d_renderer_use_cpu_palette (ezv_ctx_t ctx)
{
  ...
  glProgramUniform1i (renctx->cpu_shader, renctx->cpu_imgtex_loc,
                      EZV_CPU_TEXTURE_NUM);
  glProgramUniform1i (renctx->dapu_shader, renctx->dapu_cputex_loc,
                      EZV_CPU_TEXTURE_NUM);
}
...
void img2d_renderer_use_data_palette (ezv_ctx_t ctx)
{
  ...
  glProgramUniform1i (renctx->dapu_shader, renctx->dapu_datatex_loc,
                      EZV_DATA_TEXTURE_NUM);
}
