...
extern ezv_mon_mode_t monitoring_mode;
extern unsigned do_gmonitor_big_footprint;
...
{
  ezm_start_task (ezp_monitor, cpu);
}
...
static inline void monitoring_gpu_patch (unsigned patch, unsigned count,
                                         ...
                                         unsigned task_id)
{
  ezm_task_1D (ezp_monitor, start, end, cpu, patch, count, task_type, task_id);
}
...
{
  ezm_end_task_2D (ezp_monitor, cpu, x, y, w, h);
}
...
static inline void monitoring_end_tile_id (unsigned x, unsigned y, unsigned w,
                                           ...
                                           unsigned task_id)
{
  ezm_end_task_2D_id (ezp_monitor, cpu, x, y, w, h, task_id);
}
...
static inline void monitoring_gpu_tile (unsigned x, unsigned y, unsigned w,
                                        ...
                                        unsigned task_id)
{
  ezm_task_2D (ezp_monitor, start, end, cpu, x, y, w, h, task_type, task_id);
}
