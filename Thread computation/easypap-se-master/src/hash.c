...
void build_hash_and_compare_to_file (void *buffer, size_t len,
                                     const char *filename)
{
  char expected_hash [2 * SHA256_DIGEST_LENGTH + 1];
  char computed_hash [2 * SHA256_DIGEST_LENGTH + 1];
  ssize_t bytes_read;
  ...
  bytes_read = read (fd, expected_hash, 2 * SHA256_DIGEST_LENGTH);
  if (bytes_read != 2 * SHA256_DIGEST_LENGTH)
    exit_with_error ("Cannot read hash from \"%s\" file (expected %d bytes, "
                     "got %zd)",
                     filename, 2 * SHA256_DIGEST_LENGTH, bytes_read);
  expected_hash [2 * SHA256_DIGEST_LENGTH] = '\0';
  close (fd);

  build_hash (buffer, len, computed_hash);

  if (memcmp (expected_hash, computed_hash, 2 * SHA256_DIGEST_LENGTH) != 0)
    exit_with_error ("Hash mismatch!\n\tExpected: %s\n\tComputed: %s",
