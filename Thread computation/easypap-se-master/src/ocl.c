...
static void ocl_acquire (void)
{
  if (do_display && easypap_gl_buffer_sharing) {
    ...
    err = clEnqueueAcquireGLObjects (ocl_queue (0), 1, &tex_buffer, 0, NULL,
                                     NULL);
    ocl_check (err, "Failed to acquire lock");
  }
}
...
static void ocl_release (void)
{
  if (do_display && easypap_gl_buffer_sharing) {
    ...
    err = clEnqueueReleaseGLObjects (ocl_queue (0), 1, &tex_buffer, 0, NULL,
                                     NULL);
    ocl_check (err, "Failed to release lock");
  }
}
...
static void ocl_discover_config (int quit, int verbose)
{
  ...
  ocl_check (err, "Failed to get platform IDs");
  ...
  for (cl_uint p = 0; p < nbp && ocl_nb_gpus < MAX_GPU_DEVICES; p++) {
    ...
    ocl_check (err, "Failed to get Platform Info");
    ...
    for (cl_uint d = 0; d < nbd && ocl_nb_gpus < MAX_GPU_DEVICES; d++) {
      ...
      ocl_check (err, "Cannot get type of device");
  ...
  ocl_check (err, "Cannot get max workgroup size");
}
...
static void ocl_create_context (void)
{
  ...
  ocl_check (err, "Failed to create compute context. Please make sure OpenCL and "
...
void ocl_alloc_buffers (void)
{
  ...
  if (easypap_mode == EASYPAP_MODE_2D_IMAGES) {
    ...
    if (do_display && easypap_gl_buffer_sharing) {
      ...
      ocl_check (err, "Failed to map texture buffer\n");
    ...
    if (do_display && easypap_gl_buffer_sharing) {
      ...
      tex_buffer = clCreateFromGLBuffer (context, CL_MEM_READ_WRITE,
                                         gl_buffer_ids[0], &err);
      ocl_check (err, "Failed to map texture buffer object #0\n");
    ...
    if (ocl_nb_gpus == 1) {
      ...
      ocl_check (err, "Failed to write to neighbor_soa_buffer");
    }
  }
}
...
static void ocl_list_variants (void)
{
  ...
  ocl_check (err, "Failed to get list of kernels from program\n");

  for (int k = 0; k < kernels_found; k++) {
    size_t len;
    err = clGetKernelInfo (kernels[k], CL_KERNEL_FUNCTION_NAME, 1024, buffer,
                           &len);
    ocl_check (err, "Failed to get name of kernel\n");
...
void ocl_build_program (int list_variants)
{
  ...
  ocl_check (err, "Failed to create program");
  ...
  for (int g = 0; g < ocl_nb_gpus; g++) {
    ...
    ocl_check (err, "Failed to create compute kernel <%s>", buffer);
  }
  ...
  if (do_display && easypap_gl_buffer_sharing) {
    if (easypap_mode == EASYPAP_MODE_2D_IMAGES) {
      ...
      if (err != CL_SUCCESS) {
        ...
        ocl_check (err, "Failed to create kernel <update_texture>");
      }
    }
  }
...
void ocl_retrieve_data (void)
{
  ...
  if (easypap_mode == EASYPAP_MODE_2D_IMAGES) {
    ...
    err = clEnqueueReadBuffer (ocl_queue (0), ocl_cur_buffer (0), CL_TRUE, 0,
                               size, image, 0, NULL, NULL);
    ocl_check (err, "Failed to read from cur_buffer");
  } else {
    const unsigned size = NB_CELLS * sizeof (float);
...
void ocl_update_texture (void)
{
  ...
  if (easypap_mode == EASYPAP_MODE_2D_IMAGES) {
    ...
    err = clEnqueueNDRangeKernel (ocl_queue (0), update_kernel, 2, NULL, global,
                                  local, 0, NULL, NULL);
    ocl_check (err, "Failed to execute update_texture kernel");
    ...
    ocl_check (err, "Failed to copy data into texture buffer_object");
